...
abot = Agent(bedrock_llm,
             [get_nearest_store, get_offers_for_product, get_offers_by_retailer, add_to_shopping_cart], sqlite3_memory_checkpoint,
             system_prompt)
...
# This section is Useful in the notebook to show the state and how we can continue the conversation after interrupting the action node.
# # shows the current state of the agent,which includes all the messages.
# print(abot.graph.get_state(thread))
#
# # shows the next step at this point of time, it should be action as we added already Interrupt_before = ["action"]
# print(abot.graph.get_state(thread).next)
#
# # continue after interrupting the action node, to continue the conversation.
# # still no user input taken, it's just a continuation of the conversation.
# # passing None instead of messages, and the same thread.
